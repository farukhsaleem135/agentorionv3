import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Share2, Link2, X } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

interface ShareButtonProps {
  url: string;
  title: string;
  description?: string;
  /** URL sent to social platforms for scraping (e.g. edge function that serves OG tags). Falls back to `url`. */
  socialShareUrl?: string;
}

const platforms = [
  {
    name: "Facebook",
    icon: () => (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
    getUrl: (url: string) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`,
    color: "hover:bg-[#1877F2]/10 text-[#1877F2]",
  },
  {
    name: "LinkedIn",
    icon: () => (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
    getUrl: (url: string, title: string) => `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`,
    color: "hover:bg-[#0A66C2]/10 text-[#0A66C2]",
  },
  {
    name: "X",
    icon: () => (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
    getUrl: (url: string, title: string) => `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(title)}`,
    color: "hover:bg-foreground/10 text-foreground",
  },
];

const ShareButton = ({ url, title, description, socialShareUrl }: ShareButtonProps) => {
  const [open, setOpen] = useState(false);
  const { toast } = useToast();
  // Use the clean URL for social sharing so platforms display a friendly domain
  const shareUrlForPlatforms = url;

  const handleCopyLink = async () => {
    await navigator.clipboard.writeText(url);
    toast({ title: "Link copied!" });
    setOpen(false);
  };

  const handleShare = (getUrl: (url: string, title: string) => string) => {
    window.open(getUrl(shareUrlForPlatforms, title), "_blank", "noopener,noreferrer,width=600,height=400");
    setOpen(false);
  };

  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-card border border-border text-xs font-medium text-foreground active:scale-95 transition-transform"
      >
        <Share2 size={14} />
        Share
      </button>

      <AnimatePresence>
        {open && (
          <>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-50"
              onClick={() => setOpen(false)}
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: 8 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 8 }}
              transition={{ type: "spring", stiffness: 500, damping: 30 }}
              className="absolute right-0 top-full mt-2 z-50 bg-card border border-border rounded-2xl p-2 shadow-card min-w-[180px]"
            >
              {platforms.map((p) => {
                const Icon = p.icon;
                return (
                  <button
                    key={p.name}
                    onClick={() => handleShare(p.getUrl)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${p.color}`}
                  >
                    <Icon />
                    {p.name}
                  </button>
                );
              })}
              <div className="border-t border-border my-1" />
              <button
                onClick={handleCopyLink}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-foreground hover:bg-secondary transition-colors"
              >
                <Link2 size={18} />
                Copy Link
              </button>
            </motion.div>
          </>
        )}
      </AnimatePresence>
    </div>
  );
};

export default ShareButton;
